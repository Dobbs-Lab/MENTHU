								...
								if(!is.na(pamSites[[i]][[j]][[k]][[l]]) && !is.null(pamSites[[i]][[j]][[k]][[l]])){
									...
									if(version == 1){
										...
										if(contextCondition){
											...
											#Calculate MENTHU score
											menthuScoreFrame <- calculateMenthu2(as.character(context), cutSite = 40, weight = 20, maxdbm = 5)
											...
											if(menthuScoreFrame$menthuScore >= threshold){
												...
												#Create data frame of the current results
												formFrame  <- data.frame(Target_Sequence = crispr, 
																								 MENTHU_Score    = round(abs(menthuScoreFrame$menthuScore), digits = 2), 
